...
chmod +x ./jq
sudo cp jq /usr/bin
curl --request POST 'https://api.github.com/repos/x0rc/actions/actions/runners/registration-token' --header "Authorization: token ${personal_access_token}" > output.txt
